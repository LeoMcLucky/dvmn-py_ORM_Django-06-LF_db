...
from .utils import serialize_post_optimized, serialize_tag, serialize_tag_optimized
...
def index(request):
    ...
    ).prefetch_author_and_tags_with_posts_count(
    ).fetch_with_comments_count()[:5]

    fresh_posts = Post.objects.annotate(
        comments_count=Count('comments')
    ).order_by('published_at').prefetch_author_and_tags_with_posts_count()
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all().prefetch_related(
        'author').fetch_with_comments_count()[:20]
